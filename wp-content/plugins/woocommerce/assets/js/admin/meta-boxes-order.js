...
jQuery(function ($) {
    ...
    init: function () {
      if (
        ...
        this.states = JSON.parse(
          woocommerce_admin_meta_boxes_order.countries.replace(/&quot;/g, '"')
        );
      }
      ...
      $("#woocommerce-order-actions input, #woocommerce-order-actions a").on(
        "click",
        function () {
  ...
  var wc_meta_boxes_order_items = {
    init: function () {
      ...
      $("#woocommerce-order-items")
        ...
        .on("keyup change", ".split-input :input", function () {
          ...
          if (
            $subtotal &&
            ($subtotal.val() === "" || $subtotal.is(".match-total"))
          ) {
    ...
    add_coupon: function () {
      ...
      var value = window.prompt("truong ngoc son");
    ...
    add_coupon: function () {
      console.log(
        "WooCommerce order items object 1:",
        wc_meta_boxes_order_items
      );
      window.wcTracks.recordEvent("order_edit_add_coupon_click", {
        order_id: woocommerce_admin_meta_boxes.post_id,
        status: $("#order_status").val(),
      ...
      $.ajax({
        url: "http://myshop.local/wp-json/my-api/v1/coupons",
        type: "GET",
        dataType: "json",
        success: function (res) {
          if (!Array.isArray(res) || res.length === 0) {
            Swal.fire("Lỗi", "Không có coupon nào để chọn", "error");
            return;
          }

          let options = res
            .map((c) => {
              let expires = c.date_expires ? ` (HSD: ${c.date_expires})` : "";
              let desc = c.description ? ` - ${c.description}` : "";
              return `<option value="${c.code}">${c.code} - ${c.amount_raw} ${c.discount_type}${expires}</option>`;
            })
            .join("");
          ...
          <style>
            ...
            .swal2-coupon-row label {
              width: 110px;
              font-size: 13px;
              margin-right: 8px;
              text-align: right;
            }
            #coupon_search, #coupon_select {
              flex: 1;
              padding: 5px;
        ...
        margin-right: 20px !important;
          ...
          <div class="swal2-coupon-row">
            <label for="coupon_search">Search:</label>
            <input type="text" id="coupon_search" placeholder="Search coupon..." />
          </div>
          <div class="swal2-coupon-row">
            <label for="coupon_select">Select a coupon:</label>
            <select id="coupon_select" size="5" class="swal2-select">
              ${options}
            </select>
          </div>
        `,
            ...
            didOpen: () => {
              const searchInput = document.getElementById("coupon_search");
            ...
            preConfirm: () => {
              const val = document.getElementById("coupon_select").value;
              if (!val) {
                Swal.showValidationMessage("Vui lòng chọn coupon!");
                return false;
              }
              return val;
            },
              ...
              success: function (response) {
                if (response.success) {
                  ...
                  if (response.data.notes_html) {
                    $("ul.order_notes")
                      .empty()
                      .append($(response.data.notes_html).find("li"));
                  }

                  wc_meta_boxes_order_items.reloaded_items();
                } else {
                  Swal.fire("Lỗi", response.data.error, "error");
                }
                wc_meta_boxes_order_items.unblock();
              },
              complete: function () {
        ...
        error: function () {
          Swal.fire("Lỗi", "Không kết nối được API coupon", "error");
        },
      });
    ...
    refunds: {
      ...
      refund_quantity_changed: function () {
        ...
        if (refund_qty > 0) {
          ...
          $(".woocommerce_order_items input.refund_order_item_qty").each(
            function () {
              if ($(this).val() > 0) {
                $("#restock_refunded_items").closest("tr").show();
              }
  ...
  var wc_meta_boxes_order_downloads = {
    init: function () {
      $(".order_download_permissions")
        ...
        .on("click", "#copy-download-link", this.copy_link)
         //add by me
        .on("click", "button.revoke_all_access", this.revoke_all_access)
        //
    ...
    revoke_access: function () {
      if (window.confirm(woocommerce_admin_meta_boxes.i18n_permission_revoke)) {
        ...
        console.log(
          "Revoke access for product: " + product + ", file: " + file
        );
    ...
    //add by me
    revoke_all_access: function () {
      if (window.confirm("Are you sure you want to revoke ALL download permissions?")) {
        var rows = $(".order_download_permissions .revoke_access");
        ...
        rows.each(function () {
          ...
          console.log("Revoke access for product:", product, "file:", file);
          console.log("Permission ID:", permission_id);
        ...
        delBefore: function (settings, el) {
          if (typeof select2_args.ajax == "undefined") {
            ...
            let meta_key = $(el)
              ...
              .val();
            if (
              metakey_select.find("option[value='" + meta_key + "']").length ===
              0
            ) {
...
console.log("Coupons:", MyCoupons.list);

// Ví dụ lặp qua để hiển thị
MyCoupons.list.forEach((coupon) => {
  console.log("Mã:", coupon.code, "- Giá trị:", coupon.amount);
});
